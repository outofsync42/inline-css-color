...
var Document = new function () {
	...
	this.filePath = function (document) {
		return isset(document) ? document.uri.path.toLowerCase() : "";
	}
...
var Application = function () {
	...
	let _documentCacheEnabled = false;
	let _documentCacheEnabledParams = {
		lineCount: null
	};
	let _elementVoids = {
		area: true,
	 ...
	 * @param {vscode.TextDocument} document 
	 */
	var cacheDocumentLines = function (document) {
	 ...
	 */
	var updateCacheDocumentLines = function (document, startLine, endLine) {
	...
	*/
	...
	this.documentPath = function () {
		return Document.filePath(self.document());
	}
	this.documentType = function () {
		return Document.fileType(self.document());
	}
	this.documentLines = function () {
		return Document.lines(self.document());
	}
	...
	this.setDocumentCacheEnabled = function (params) {
		if (is_object(params)) {
			for (var i in params) {
				_documentCacheEnabledParams[i] = params[i];
			}
		} else {
			_documentCacheEnabled = is_bool(params) ? params : true;
		}
	}
	...
	/**
	 * @param {vscode.DocumentSelector} language 
	 * @param {registerCompletionProvider} callback 
	 * @param {Array} chars 
	 */
	...
	var documentOpen = function (document) {
		...
		_documents[filePath] = {
			...
			cacheEnabled: _documentCacheEnabled,
		...
		//overrides
		if (isset(_documentCacheEnabledParams.lineCount)) {
			_documents[filePath]['cacheEnabled'] = document.lineCount >= _documentCacheEnabledParams.lineCount
		}
	...
	var textDocumentChange = function () {
		try {
			if (_textDocumentChangeQueue.length > 0) {
				...
				event.diff = diff;
				if (isset(_documentCacheEnabledParams.lineCount)) {
					_documents[filePath]['cacheEnabled'] = _documents[filePath]['lineCount'] >= _documentCacheEnabledParams.lineCount
				}
...
/**
 * @callback registerCompletionProvider
 * @param {vscode.TextDocument} document 
 * @param {vscode.Position} position 
 * @param {vscode.CancellationToken} token 
 * @param {vscode.CompletionContext} context 
